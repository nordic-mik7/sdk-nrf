...
/**
 * @brief Interface functions for external lzma dictionary, see nrf_compress/lzma_types.h.
 */
...
static int open_dictionary(size_t dict_size, size_t *buff_size)
{
	/** We always return success in here.
	 * If requested dict_size is bigger than our decompressed image
	 * size we fake *buff_size to be enough, because dictionary is never
	 * accessed past the size of the decompressed data.
	 *
	 * If, for any reason, dictionary was accessed outside the bounds of
	 * output_sink memory, write_dictionary and read_dictionary
	 * interfaces would return an error and entire decompression process
...
static suit_plat_err_t used_storage(void *ctx, size_t *size)
{
	...
	if (decompress_ctx->out_sink.used_storage != NULL) {
		return decompress_ctx->out_sink.used_storage(
					decompress_ctx->out_sink.ctx, size);
	}
...
static suit_plat_err_t erase(void *ctx)
{
	...
	if (ctx != NULL) {
		...
		(void)codec_impl->reset((void *)decompress_ctx->codec_ctx);
		...
		if (decompress_ctx->out_sink.erase != NULL &&
			size_ret == SUIT_PLAT_SUCCESS && size != 0) {
...
static suit_plat_err_t write(void *ctx, const uint8_t *buf, size_t size)
{
	...
	while (size + decompress_ctx->last_chunk_size > CHUNK_BUFFER_SIZE) {
		...
		if (decompress_ctx->last_chunk_size != 0) {
			...
			/* Following check is for the header case - 2 bytes may be requested by codec. */
			...
			if (decompress_ctx->last_chunk_size != 0) {
				/* Shift unprocessed data in last chunk
				 * to the buffer beginning. */
				memcpy(decompress_ctx->last_chunk,
					decompress_ctx->last_chunk + chunk_size, decompress_ctx->last_chunk_size);
			}
			continue;
		}
...
static suit_plat_err_t flush(void *ctx)
{
	...
	if (decompress_ctx->last_chunk_size < CHUNK_BUFFER_SIZE) {
		LOG_WRN("Wrong decompression state. Error during decompression or too little input data provided.");
						   ...
						   enum nrf_compress_types *compress_type)
{
	...
	{
		case suit_lzma2:
			/** TODO: Perform the validation of class_id ?*/
					...
					out_sink_readback_func out_sink_readback)
{
	...
	if (compress_info == NULL || out_sink == NULL || in_sink == NULL ||
	    class_id == NULL || out_sink_readback == NULL  || out_sink->write == NULL ||
