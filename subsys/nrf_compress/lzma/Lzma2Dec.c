...
static void LzmaDec_UpdateWithUncompressed(CLzmaDec *p, const Byte *src, SizeT size)
{
	...
	LzmaDictionaryWrite(p->dicHandle, p->dicPos, src, size);
#else
...
ELzma2ParseStatus Lzma2Dec_Parse(CLzma2Dec *p, SizeT outSize, const Byte *src, SizeT *srcLen,
	...
	while (p->state != LZMA2_STATE_ERROR) {
		...
		if (p->state != LZMA2_STATE_DATA && p->state != LZMA2_STATE_DATA_CONT) {
			...
			if (outSize == 0 && p->state != LZMA2_STATE_FINISHED) {
				...
				// finished, We can return LZMA_STATUS_NOT_SPECIFIED or
				// LZMA_STATUS_NOT_FINISHED here break;
				return (ELzma2ParseStatus)LZMA_STATUS_NOT_FINISHED;
			}
