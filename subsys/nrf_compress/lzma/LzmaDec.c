...
Processing:
  ...
  that is enough for worst case LZMA symbol with one additional RangeCoder normalization for one
bit. So that function never reads bufLimit [LZMA_REQUIRED_INPUT_MAX] byte.
...
static int Z7_FASTCALL LZMA_DECODE_REAL(CLzmaDec *p, SizeT limit, const Byte *bufLimit)
{
...

		{
			...
			else
			{
				...
				IF_BIT_0(prob)
				{
					...
					IF_BIT_0(prob)
					{
						...
						// if (checkDicSize == 0 && processedPos == 0) { len
						// = kMatchSpecLen_Error_Data + 1; break; } The
						// caller doesn't allow (dicPos == limit) case here
...

			{
				...
				if ((rem = limit - dicPos) == 0) {
					/*
					We stop decoding and return SZ_OK, and we can resume
					decoding later. Any error conditions can be tested later in
					caller code. For more strict mode we can stop decoding with
					error
					// len += kMatchSpecLen_Error_Data;
					*/
					break;
	...
	p->range = range;
	p->code = code;
	p->remainLen = (UInt32)len; // & (kMatchSpecLen_Error_Data - 1); // we can write real length
				    // for error matches too.
	p->dicPos = dicPos;
...
#define kBadRepCode                                                                                \
	(kBound0 + (((kRange0 - kBound0) >> kNumBitModelTotalBits) << (kNumBitModelTotalBits - 1)))
...
static ELzmaDummy LzmaDec_TryDummy(const CLzmaDec *p, const Byte *buf, const Byte **bufOut)
{
	...
	for (;;) {
		...
		unsigned ttt;
		unsigned posState =
			CALC_POS_STATE(p->processedPos, ((unsigned)1 << p->prop.pb) - 1);
...
When the decoder reaches dicLimit, it looks (finishMode) parameter:
  ...
  if (finishMode != LZMA_FINISH_ANY), the decoder lookahead, if end_marker is possible for current
position
...

		{
			...
			if (p->tempBufSize == 0) {
				...
				if (inSize < LZMA_REQUIRED_INPUT_MAX || checkEndMarkNow) {
					...
					if (checkEndMarkNow &&
					    !IS_DUMMY_END_MARKER_POSSIBLE(dummyRes)) {
						unsigned i;
...

			{
				...
				// in strict mode: tempBufSize is not enough for one Symbol
				// decoding. in relaxed mode: tempBufSize not larger than required
				// for one Symbol decoding.
				...
				if (rem < LZMA_REQUIRED_INPUT_MAX || checkEndMarkNow) {
					...
					ELzmaDummy dummyRes =
						LzmaDec_TryDummy(p, p->tempBuf, &bufOut);
					...
					if (checkEndMarkNow &&
					    !IS_DUMMY_END_MARKER_POSSIBLE(dummyRes)) {
						(*srcLen) +=
							(unsigned)dummyProcessed - p->tempBufSize;
						p->tempBufSize = (unsigned)dummyProcessed;
						// p->remainLen = kMatchSpecLen_Error_Data;
						RETURN_NOT_FINISHED_FOR_FINISH
					}
				...
				{
					// we decode one symbol from (p->tempBuf) here, so the
					// (bufLimit) is equal to (p->buf)
		...
		ISzAllocPtr alloc)
{
	...
	*status = LZMA_STATUS_NOT_SPECIFIED;
	...
	LzmaDec_CONSTRUCT(&p) RINOK(LzmaDec_AllocateProbs(&p, propData, propSize, alloc)) p.dic =
		dest;
